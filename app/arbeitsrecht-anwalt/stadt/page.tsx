...
export default function ArbeitsrechtAnwaltPage() {
  const byRegion = regionen.reduce((acc, r) => {
    acc[r] = staedte.filter((s) => s.region === r);
    return acc;
  }, {} as Record<Region, typeof staedte>);
        ...
        <div key={region} className="mb-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
            {byRegion[region].map((stadt) => (
              ...
              >
                <p className="text-sm font-medium text-gray-900 group-hover:text-[#6B6626] mb-0.5">{stadt.name}</p>
                <p className="text-xs text-gray-400">{stadt.bundesland}</p>
              </a>
            ))}
          </div>
        </div>
